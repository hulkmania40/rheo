...
const SideMenu = ({ seatDetails }) => {
  ...
  return (
    <div className="sidemenuWrp">
      ...
      <div className='title'>theatres
        <div className='subtitle'>
          <div>devicineplex |</div>
          <div>devi bala</div>
        </div>
      </div>
      <div className='title'>showtime
        <div className='subtitle'>
      ...
      <div className="sdmnprcnt">
        <p>{seatDetails.numberOfSeats?seatDetails.numberOfSeats:0} Seat(s)</p>
        <span>&#8377; {seatDetails.price?seatDetails.price.toFixed(2):0}</span>
      </div>
    </div>
  );
};
